...
fail() {
  echo "cloud observability capacity check failed: $*" >&2
  exit 1
}
...
OBS_ENABLED="$(python3 "${SCRIPT_DIR}/Component/lib/read-cloud-setting.py" observability_enabled 2>/dev/null || echo false)"

if [[ "$OBS_ENABLED" != "true" ]]; then
  echo "cloud observability capacity check skipped: observability_enabled is not true"
  exit 0
fi
...
echo "checking Cloud observability capacity"
check_group load_balancer 1536 384 10240
check_group app_servers 256 96 2048
check_group node_db 512 192 4096

echo "cloud observability capacity check ok"
